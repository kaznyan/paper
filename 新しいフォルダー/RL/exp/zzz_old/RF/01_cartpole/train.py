...
def main():
    ...
    ### 2つのNWを作成・初期化
    online_net = RFR(num_inputs, num_actions)
    ...
    for e in range(3000):
        ...
        score = 0
        state = env.reset()
        state = np.asarray(state).reshape((1, num_inputs))
        ...
        while not done:
            ...
            next_state = np.asarray(next_state).reshape((1, num_inputs))
            ...
            if steps > initial_exploration:
                ...
                # loss = RFR.train_model(online_net, target_net, None, batch)
